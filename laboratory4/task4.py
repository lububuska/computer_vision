...
def task4(smoothed_matrix_normalized):
    T_high = 0.2 * smoothed_matrix_normalized.max()
    T_low = 0.4 * T_high
    ...
    result_matrix = np.zeros((height, width),dtype = float)
    
    for i in range(height):
        for j in range(width):
            value = smoothed_matrix_normalized[i, j]
    
            if value >= T_high:
                result_matrix[i, j] = strong
            elif value >= T_low:
                result_matrix[i, j] = weak
            else:
                result_matrix[i, j] = 0
                
    for i in range(1, height - 1):
        for j in range(1, width - 1):
            neighbours = [result_matrix[i - 1, j - 1], result_matrix[i - 1, j], result_matrix[i - 1, j + 1], result_matrix[i, j + 1], result_matrix[i + 1, j + 1], result_matrix[i + 1, j], result_matrix[i + 1, j - 1], result_matrix[i, j - 1]]
            if result_matrix[i, j] == weak:
                if 255 in neighbours:
                    result_matrix[i, j] = strong
                else:
                    result_matrix[i, j] = 0
                
    cv2.imshow("Canny detection", np.uint8(smoothed_matrix_normalized))
...
result = task4(smoothed_matrix_normalized)